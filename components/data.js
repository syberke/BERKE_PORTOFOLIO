// ─── SHARED DATA & HELPERS ───────────────────────────────────────────────
// Centralized data + small presentational helpers used across sections.
// Keeping this in one place avoids duplication and shrinks each component.
...
export const MOTION_PATH_D =
  "M 0,150 C 150,50 300,250 450,150 S 650,50 800,150 S 950,250 1100,150";
/* ─── Presentational helpers ─────────────────────────────────────────── */
export const W = { maxWidth: 1200, margin: "0 auto" };
export const sec = (bg) => ({ position: "relative", padding: "8rem 3rem", background: bg });
...
export function SHead({ label, title, center }) {
  return (
    <div style={{ marginBottom: "3.5rem", textAlign: center ? "center" : "left" }}>
      {label && <SLabel text={label} />}
      <h2
        style={{
